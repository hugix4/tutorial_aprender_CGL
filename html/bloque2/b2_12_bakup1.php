...
if(isset($_POST["enviar1"])){
		...
		if($enviadas==$esperadas){
			$sql_guardar="INSERT INTO cuestionario_b2_12_1( id_alumno, tipo_usuario, fecha_hora, p1,p2,p3,p4,p5) VALUES( '$numcta', $tipoUsuario, NOW(), $p1,$p2,$p3,$p4,$p5);";
			//echo $sql_guardar;
			$conexion->ejecutar($sql_guardar);
		}
}

$sql_revisar="SELECT COUNT(id_alumno) as 'respuestas' FROM cuestionario_b2_12_1 WHERE id_alumno='$numcta' LIMIT 1;"; 
...
<div class="spacerer">&nbsp;</div>
          <div class="btnresp">
           <div class="medium default btn">
              <input type="submit" value="Enviar respuestas" name="enviar1" />
           </div>
          </div>
...
if(isset($_POST["enviar2"])){
		if(isset($_POST["p6"])){$p6=$_POST["p6"]; $enviadas2++;}else{ $mensaje_error2.="Falta contestar la pregunta 1<br>";};
		if(isset($_POST["p7"])){$p7=$_POST["p7"]; $enviadas2++;}else{ $mensaje_error2.="Falta contestar la pregunta 2<br>";};
		if(isset($_POST["p8"])){$p8=$_POST["p8"]; $enviadas2++;}else{ $mensaje_error2.="Falta contestar la pregunta 3<br>";};
		if(isset($_POST["p9"])){$p9=$_POST["p9"]; $enviadas2++;}else{ $mensaje_error2.="Falta contestar la pregunta 4<br>";};
		if(isset($_POST["p10"])){$p10=$_POST["p10"]; $enviadas2++;}else{ $mensaje_error2.="Falta contestar la pregunta 5<br>";};
		if(isset($_POST["p11"])){$p11=$_POST["p11"]; $enviadas2++;}else{ $mensaje_error2.="Falta contestar la pregunta 6<br>";};
		
		if($enviadas2==$esperadas2){
			$sql_guardar2="INSERT INTO cuestionario_b2_12_2( id_alumno, tipo_usuario, fecha_hora, p1,p2,p3,p4,p5,p6) VALUES( '$numcta', $tipoUsuario, NOW(), $p6,$p7,$p8,$p9,$p10,$p11);";
			//echo $sql_guardar2;
			$conexion->ejecutar($sql_guardar2);
		}
}

$sql_revisar2="SELECT COUNT(id_alumno) as 'respuestas' FROM cuestionario_b2_12_2 WHERE id_alumno='$numcta' LIMIT 1;"; 
...
?>
    <form action="#" method="post" id="miform">
      <label for="p6" class="radio">&iquest;Estudias o trabajas siempre a la <strong>misma hora</strong> y <strong>todos los d&iacute;as</strong>?</label>
      <div class="respuestas">
        ...
        No </div>
      <label for="p7" class="radio">&iquest;Programas &nbsp;el estudio de acuerdo con las exigencias y la <strong>dificultad</strong> de cada materia? </label>
      <div class="respuestas">
        ...
        No </div>
      <label for="p8" class="radio">&iquest;Estudias <strong>todos  los d&iacute;as</strong> durante un poco de tiempo, en vez de estudiar mucho tiempo durante  muy pocos d&iacute;as?</label>
      <div class="respuestas2">
        ...
        No </div>
      <label for="p9" class="radio">&iquest;Te pones de <strong>acuerdo con tu familia y los amigos</strong> para que no te interrumpan en  el horario de estudio?</label>
      <div class="respuestas2">
        ...
        No </div>
      <label for="p10" class="radio">&iquest;<strong>Duermes</strong> 8 horas diarias? </label>
      <div class="respuestas">
        ...
        No </div>
      <label for="p11" class="radio">&iquest;Tienes suficiente tiempo para <strong>actividades placenteras</strong>? </label>
      <div class="respuestas">
...
<div class="spacerer">&nbsp;</div>
          <div class="btnresp">
           <div class="medium default btn">
              <input type="submit" value="Enviar respuestas" name="enviar2" />
           </div>
          </div>
